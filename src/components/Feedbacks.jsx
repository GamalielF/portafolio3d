  ...
  <motion.div variants={fadeIn("","spring", index * 0.5, 0-75)} className='bg-black-200 p-10 rounded-3xl xs:w-[320px] w-full' >
    ...
    <div className='mt-1'>
      ...
      <div className='mt-7 flex justify-between items-center gap-1'>
        ...
        <img src={image} alt={`feedbak-by-${name}`} className='w-10 h-10 rounded-full object-cover' />
      </div>
    </div>
  </motion.div>
)
    ...
    <div className='mt-12 bg-black-100 rounded-[20px]'>
      <div className={`${styles.padding} bg-tertiary rounded-2xl min-h-[300px]`}>
        <motion.div variants={textVariant()}>
          <p className={styles.sectionSubText} >¿Que dicen de mi trabajo?</p>
          <h2 className={styles.sectionHeadText} >Testimonios.</h2>
        </motion.div>
        <div className={`${styles.paddingX}  pb-14 flex flex-wrap gap-10`}  >
